...
- (void)calculateAllItemsOrigin {
    ...
    for (int i = 0; i < numberOfColumns; i++) {
//        NSLog(@"viewOrigin:%f",viewOrigin);
        [ay addObject:[NSNumber numberWithFloat:viewOrigin]];
//        [originPointList addObject:[NSNumber numberWithFloat:viewOrigin]];
//        NSLog(@"iiiiii:%d    viewOrigin:%f",i,[[originPointList objectAtIndex:i] floatValue]);
//        NSLog(@"iiiiii:%d    ay viewOrigin:%f",i,[[ay objectAtIndex:i] floatValue]);
        viewOrigin += [viewDelegate columnView:self widthForColumnAtIndex:i];
    }
    [originPointList removeAllObjects];
    [originPointList addObjectsFromArray:ay];
    [ay release];
}
...
//    NSLog(@"numberOfColumns:%d",numberOfColumns);
    for (int i = 0; i < numberOfColumns; i++) {
//        NSLog(@"iiiiiii:%d  originList:%f",i,[[originPointList objectAtIndex:i] floatValue]);
...
//    NSLog(@"calculateItemIndexRange startIndex:%d",startIndex);
//    NSLog(@"calculateItemIndexRange endIndex:%d",endIndex);
}
...
//    NSLog(@"onScreenViewDic count:%d",[onScreenViewDic count]);
