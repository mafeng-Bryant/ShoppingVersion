...
-(void)accessItemService
{
    ...
    {
        reqUrl = @"index/specialtagdetail.do?param=%@";
    }
    else
    {
        reqUrl = @"product/specialOfferlist.do?param=%@";
    }
	...
	[[DataManager sharedManager] accessService:jsontestDic
									   command:OPERAT_SPECIAL_TAGS_LIST_REFRESH 
								  accessAdress:reqUrl 
									  delegate:self
									 withParam:nil];
}
...
-(void)accessMoreService
{
	...
	[[DataManager sharedManager] accessService:jsontestDic
									   command:OPERAT_SPECIAL_TAGS_LIST_MORE 
								  accessAdress:reqUrl 
									  delegate:self
									 withParam:nil];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
	...
	if (countItems > [indexPath row]) 
    {
        ...
        if (contentType == 1) {
            ProductDetailView.pvType = @"3";
            ProductDetailView.pvType3_Id = self.specialId;
        }else
        {
