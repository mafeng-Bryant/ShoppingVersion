...
- (void)setPageAtIndex:(NSInteger)pageIndex{
    ...
    if ((NSObject *)cell == [NSNull null]) {
        ...
        btn.tag = pageIndex + 1;
        [btn addTarget:self action:@selector(touchView:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)touchView:(id)sender
{
    ...
    if ([_delegate respondsToSelector:@selector(touchViewCellForPageAtIndex:)])
    {
        [_delegate touchViewCellForPageAtIndex:btn.tag - 1];
    }
