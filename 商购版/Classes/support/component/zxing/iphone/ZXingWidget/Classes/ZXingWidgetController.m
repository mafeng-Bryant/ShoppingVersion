...
@synthesize oneDMode , isStatusBarHidden;
@synthesize readers;
...
- (id)initWithDelegate:(id<ZXingDelegate>)scanDelegate OneDMode:(BOOL)shouldUseoOneDMode {
  ...
  if (self) {
    ...
    beepSound = -1;
    decoding = NO;
    OverlayView *theOverLayView = [[OverlayView alloc] initWithFrame:[UIScreen mainScreen].bounds oneDMode:oneDMode];
...
- (void)goHistory {
    ...
    if (delegate != nil) 
    {
        [delegate zxingControllerGoHistory:self];
    }
}
...
- (void)notification:(NSNotification*)notification 
{
    ...
    {
        [delegate zxingControllerDidStartRunning:self];
    }
}
