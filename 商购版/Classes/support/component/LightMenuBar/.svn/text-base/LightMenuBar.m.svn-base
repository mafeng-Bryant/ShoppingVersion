...
//左右滚动
- (void)goLeftOrRight:(NSString *)leftOrRight animated:(BOOL)animated
{
    [_menuBarView setNeedsDisplay];
    
    if ([leftOrRight isEqualToString:@"left"]) 
    {
        CGFloat _scrollViewX = _scrollView.contentOffset.x - (self.frame.size.width/1);
        if (_scrollViewX > 0) 
        {
            [_scrollView setContentOffset:CGPointMake(_scrollViewX, 0) animated:animated];
        }
        else 
        {
    ...
    {
        CGFloat _scrollViewX = _scrollView.contentOffset.x + (self.frame.size.width/1);
        if ((_menuBarView.barLength - _scrollViewX) > self.frame.size.width) 
        {
            [_scrollView setContentOffset:CGPointMake(_scrollViewX, 0) animated:animated];
        }
        else 
        {
            [_scrollView setContentOffset:CGPointMake(_menuBarView.barLength - self.frame.size.width, 0) animated:animated];
        }
    }
}
...
- (void)setup {
    ...
    _menuBarView.backgroundColor = [UIColor clearColor];
